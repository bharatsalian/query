view: addresses {
  sql_table_name: mem_persona.addresses ;;
  ...
  dimension: full_address {
    type: string
    sql:coalesce(${TABLE}.address_1 ||' ' ,'') ||
         coalesce(${TABLE}.address_2 || ' ','') ||
        coalesce(${TABLE}.address_3 || ' ','') || ';'|| ${city}|| ' '|| ${state} || ' '
    ||(case when ${country} ='US' then ltrim(${zip}::varchar,'000000000') else ${postal_code} end)|| ' ' ||${country} ;;
  }
  dimension: address_1 {
    type: string
    sql: ${TABLE}.address_1 ;;
